...
For a compact transition table and invariants, see [`../models/order-lifecycle.md`](../models/order-lifecycle.md).
...
- `private_key_policy`: states whether key material is service-generated, supplied by CSR, or not stored; for the v1, prefer a small explicit set such as `service_generated` and `csr_only`
- `csr_source`: states whether the request supplied a CSR or expects the service to generate key material and CSR; for the v1, prefer a small explicit set such as `client_provided` and `service_generated`
...
Normalization and deduplication rules for the v1:
...
Do not turn these fields into large plugin-style registries or open-ended free-form values in the v1 unless a later iteration proves the need.
...
For the v1, state-transition history may be represented either by:
...
Do not require a separate transition-history table in the v1 unless the implementation has a real query or recovery need that audit events cannot satisfy cleanly.

### Retry and expiration record

For the v1, keep retry and expiration tracking on the order row.
...
Expiration rules for the v1:

- set `expires_at` when the order is created
- use one bounded lifetime policy for the v1 rather than per-plugin expiration logic
...
Retry classification rules for the v1:
...
For the v1, prefer a small schema with a few well-chosen tables:
...
Recommended claim algorithm for the v1:
...
Do not introduce a separate queue or lease-management subsystem for the v1.

Because ACME is part of the primary v1 surface, add only the persistence needed to support the documented ACME contract:
...
Do not add ACME-specific persistence beyond the documented contract unless the implementation has reached a later ACME expansion slice described in [`implementation-roadmap.md`](../guides/implementation-roadmap.md).
...
Whichever strategy is chosen, it must support the nonce behavior documented in [`acme-api.md`](../reference/acme-api.md).
...
For the v1, keep the admin API intentionally small.
...
Do not add write-heavy or lifecycle-mutating admin endpoints in the v1 unless a later document explicitly requires them.

For concrete broker and admin request or response behavior, use [`broker-api.md`](../reference/broker-api.md).

### 5.4 Health surface

For the v1, keep health endpoints minimal.
...
For the broker configuration shape and policy matching rules, use [`configuration.md`](../reference/configuration.md).

For ACME-visible object and status behavior, use [`acme-api.md`](../reference/acme-api.md).

For the later broker-native HTTP contract, use [`broker-api.md`](../reference/broker-api.md).
